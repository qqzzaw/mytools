...
with st.sidebar:
    ...
    try:
        ...
        st.markdown(f"""
            <div style="background: linear-gradient(135deg, #1e1e2f 0%, #0e1117 100%); 
                ...
                <p style="margin: 5px 0 0 0; color: #555; font-size: 0.7rem;">{cfg['tz']}</p>
            </div>
        """, unsafe_allow_html=True)
    except: st.caption("Time Syncing...")
    ...
    try:
        ...
        options = [f"{row['icon']} {row[l_col]}" for _, row in df.iterrows()]
...
if current_id == "m1": # 二维码
        ...
        if st.button("Execute"):
            ...
            for i in range(3):
                for j in range(3):
                    ...
                    cols[j].download_button(f"#{i*3+j+1}", b.getvalue(), f"p.jpg")

elif current_id == "m3": # TikTok 标签
    kw = st.text_input("TikTok Topic", "Viral")
    ...
    vf = st.file_uploader("Upload Video", type=["mp4", "mov"])
    if vf:
        st.video(vf)
        st.write(f"📁 **Size:** {round(vf.size / 1024 / 1024, 2)} MB")

elif current_id == "m7": # 智能笔记
    note = st.text_area("Write notes...", height=150)
    if note:
        st.download_button("Save TXT", note, "note.txt")

elif current_id == "m8": # 星座运势 (对应 CSV 里的第八个)
...
st.caption("© 2024 GengTools™ Global | All Tools Active")
